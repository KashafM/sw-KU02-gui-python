...
void MPU_accelgyro() {
  ...
  AccX = (float(rawAccel.XAxis) / 16384.0) - error_Ax;
  AccY = (float(rawAccel.YAxis) / 16384.0) - error_Ay;
  AccZ = (float(rawAccel.ZAxis) / 16384.0) - error_Az;
  // Sample gyroscope data
  Vector rawGyro = mpu.readRawGyro(); // Read in the raw gyroscope voltages
  //Vector normGyro = mpu.readNormalizeGyro();
  GyroX = (float(rawGyro.XAxis) / 131.0) - error_Gx;
  GyroY = (float(rawGyro.YAxis) / 131.0) - error_Gy;
  GyroZ = (float(rawGyro.ZAxis) / 131.0) - error_Gz;
...
void calibrate_IMU() {
  ...
  while (samp_count_cal < 200) {
    ...
    // Sum all readings
    error_Ax = error_Ax + (AccX / 16384.0); //; ((atan((AccY) / sqrt(pow((AccX), 2) + pow((AccZ), 2))) * 180 / PI));
    error_Ay = error_Ay + (AccY / 16384.0); // ((atan(-1 * (AccX) / sqrt(pow((AccY), 2) + pow((AccZ), 2))) * 180 / PI));
    error_Az = error_Az + (AccZ / 16384.0);
    delay(5);
    samp_count_cal++;
  }
  ...
  while (samp_count_cal < 200) {
    ...
    // Sum all readings
    error_Gx = error_Gx + (GyroX / 131.0);
    error_Gy = error_Gy + (GyroY / 131.0);
    error_Gz = error_Gz + (GyroZ / 131.0);
    delay(5);
    samp_count_cal++;
  }
  //Divide the sum by 200 to get the error value
