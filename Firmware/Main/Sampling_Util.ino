...
void SampleEOG() {
  ...
  adcval1.x = analogRead(adcin1); //Read and convert the analog value from pin A0
  adcval2.x = analogRead(adcin2); //Read and convert the analog value from pin A1
}

// ISR function for sampling EOG, temperature, and accelerometer data with the following sampling periods: 5 ms, 10 ms, and 1 ms
void SampleAll() {
  SampleEOG(); 
  current_chunk_sample.EOG1[total_chunk_samp].x = adcval1.x;
  current_chunk_sample.EOG2[total_chunk_samp].x = adcval2.x;
  ...
  if (sample_IMU == 2) {
    ...
    current_chunk_sample.GyroX[sample_count_IMU].x = GyroX.x;
    current_chunk_sample.GyroY[sample_count_IMU].x = GyroY.x;
    current_chunk_sample.GyroZ[sample_count_IMU].x = GyroZ.x;
    sample_IMU = 0;
  ...
  if (sample_count_IMU == 100) {
    ...
    current_chunk_sample.temp.x = raw_t.x;
